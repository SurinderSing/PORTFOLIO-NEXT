...
1. **REST API Routes & Dedicated Services (`/api/...` & `src/services/`):**
   ...
   - **Debounced Interactions & AbortController Cancellation:** Added `debouncedTogglePostLike` and `debouncedToggleCommentReaction` in `src/services/blogApi.ts` with 350ms batching and in-flight request cancellation. Updated backend endpoints (`/api/blog/like` and `/api/blog/reaction`) to support idempotent target states (`desiredLiked` and `desiredReaction`). Rapid clicking is now 100% fluid with zero lag or race conditions.
