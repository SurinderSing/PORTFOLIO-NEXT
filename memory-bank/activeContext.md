...
1. **REST API Routes & Dedicated Services (`/api/...` & `src/services/`):**
   ...
   - **Home Page Auto-Scroll / Jump Fix:** Fixed the issue where navigating to `/` (Home) auto-scrolled directly down to Featured Work. The root cause was the embedded live preview iframe for `gimmefy.ai` executing `input.focus()` on load; resolved by hardening `sandbox="allow-scripts"` (removing `allow-same-origin` / `allow-forms`) and adding `tabIndex={-1}` / `aria-hidden="true"` so the iframe cannot steal parent window focus.
