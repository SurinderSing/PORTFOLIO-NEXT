import Image from 'next/image';
import React from 'react';
import Photo from '@/assets/images/profile-photos/surinder_profile_photo.png';
import { getSiteSettings } from '@/lib/supabase-queries';

const PhotoSection: React.FC = async () => {
  const settings = await getSiteSettings();

  return (
    <div className="flex flex-col items-center w-full max-w-[70%]">
      <div className="relative w-[150px] h-[74px]">
        <Image
          src={settings.profile_photo_url || Photo}
          alt={`${settings.owner_name} - ${settings.owner_title}`}
          width={150}
          height={150}
          className="object-cover aspect-square rounded-full absolute -top-[82px]"
          priority
        />
      </div>
      <h3 className="mb-1 text-[22px]">{settings.owner_name}</h3>
      <p className="para-2 font-semibold text-muted">{settings.owner_title}</p>
    </div>
  );
};

export default PhotoSection;
